...
import {
  ...
  AppointmentMessage,
} from './process-appointment.use-case';
...
export class ResilientProcessAppointmentUseCase {
  ...
  async executeWithResilience(
    ...
    try {
      ...
      this.logger.log(
        `Appointment processed successfully with Saga: ${message.appointmentId}`,
      );
    } catch (error) {
      this.logger.error(
        `Resilient appointment processing failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
  ...
  private async validateOrCreatePatient(
    message: AppointmentMessage,
  ): Promise<Patient> {
    let patient = await this.patientRepository.findByEmail(
  ...
  private async saveAppointmentWithOutbox(
    message: AppointmentMessage,
  ): Promise<string> {
    const scheduledDate = parseISO(message.scheduledAt);
    ...
    this.logger.log(
      `Appointment saved with Outbox Pattern: ${message.appointmentId}`,
    );
  ...
  private async deleteAppointment(appointmentId: string): Promise<void> {
    try {
      ...
      this.logger.error(
        `Failed to compensate appointment deletion: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
    }
  }
  ...
  private async sendConfirmationNotification(
    ...
    this.logger.log(
      `Sending confirmation notification for appointment: ${message.appointmentId}`,
    );
  ...
  private async sendCancellationNotification(
    ...
    this.logger.log(
      `Compensating: sending cancellation notification for appointment: ${message.appointmentId}`,
    );
